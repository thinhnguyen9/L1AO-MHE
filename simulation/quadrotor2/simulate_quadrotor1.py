"""
LMHE1/2/3/4 are all L1AO with horizon N=10/20/50/100.
"""
...
def main(
    ...
    for loop in range(loops):
        ...
        if 'LMHE1' in enabled_estimators:
            ...
            LMHE1_obj = MHE(
                ...
                xs              = xhover_est,
                us              = uhover_est,
                pcip_obj        = lmhe1_pcip_obj,
                l1ao_obj        = lmhe1_l1ao_obj
            )
        ...
        if 'LMHE4' in enabled_estimators:
            ...
            LMHE4_obj = MHE(
                ...
                xs              = xhover_est,
                us              = uhover_est,
                pcip_obj        = lmhe4_pcip_obj,
                l1ao_obj        = lmhe4_l1ao_obj
            )
        ...
        if 'LMHE4' in enabled_estimators:
            xhat_lmhe4, lmhe4_time = sim.run_estimation(LMHE4_obj, x0)
            rmse_lmhe4 = rmse(xvec[t0:], xhat_lmhe4[t0:])
            if keep_initial_guess: xhat_lmhe4[0] = x0.copy()
        if 'NMHE' in enabled_estimators:
        ...
        if 'LMHE1' in enabled_estimators:   print(f"LMHE1 (N=10)      : {rmse_lmhe1:.4f}\t\t{lmhe1_time*1000./N:.4f}")
        if 'LMHE2' in enabled_estimators:   print(f"LMHE2 (N=20)      : {rmse_lmhe2:.4f}\t\t{lmhe2_time*1000./N:.4f}")
        if 'LMHE3' in enabled_estimators:   print(f"LMHE3 (N=50)      : {rmse_lmhe3:.4f}\t\t{lmhe3_time*1000./N:.4f}")
        if 'LMHE4' in enabled_estimators:   print(f"LMHE4 (N=100)     : {rmse_lmhe4:.4f}\t\t{lmhe4_time*1000./N:.4f}")
        ...
        if save_csv_simulation_instance:
            ...
            if 'LMHE4' in enabled_estimators:   data.append([run_id, loop+1, 'LMHE4', rmse_lmhe4, np.max(np.abs(xvec-xhat_lmhe4)), lmhe4_time*1000./N, T, ts, t0])
        ...
        if save_csv_estimation_error:
            ...
            if 'LMHE4' in enabled_estimators:   err_lmhe4 = np.linalg.norm(xvec - xhat_lmhe4, axis=1)
            ...
            with open(file_path, 'a', newline='') as csvfile:
                ...
                for i in range(N):
                    ...
                    if 'LMHE4' in enabled_estimators:   writer.writerow([run_id, loop+1, 'LMHE4', T, f"{t:.2f}", f"{err_lmhe4[i]:.6f}"])
    ...
    file_path = os.path.join(os.path.dirname(__file__), f'raw_data/{run_id}.npy')
    with open(file_path, 'wb') as f:
        ...
        np.save(f, yvec)
        np.save(f, uvec)
        if 'EKF' in enabled_estimators:     np.save(f, xhat_ekf)
        if 'LMHE1' in enabled_estimators:   np.save(f, xhat_lmhe1)
        if 'LMHE2' in enabled_estimators:   np.save(f, xhat_lmhe2)
        if 'LMHE3' in enabled_estimators:   np.save(f, xhat_lmhe3)
        if 'LMHE4' in enabled_estimators:   np.save(f, xhat_lmhe4)
